...
export default function Contact() {
  ...
  const contactLinks = [{
    ...
    href: "mailto:m7xvoltx@gmail.com", icon: <Mail className="w-5 h-5" />
  },
  {
    platform: "LinkedIn",
  ...
  return (
    <section id="contact" className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50">
      <div className="bg-white/90 dark:bg-[#0d0d0d]/80 backdrop-blur-sm border border-gray-200 dark:border-white/20 rounded-2xl py-3 px-6 shadow-lg transition-colors duration-300">
        <div className="flex items-center space-x-4">
          <div className="flex space-x-4 items-center">
            {contactLinks.map((contact, index) => (<a
              key={index}
              href={contact.href}
              target="_blank"
          ...
          <div className="w-px h-6 bg-gray-300 dark:bg-white/20"></div>          <button
            onClick={toggleTheme}
            className="text-gray-700 dark:text-gray-300 hover:text-black dark:hover:text-white transition-colors p-1 hover:bg-gray-100 dark:hover:bg-white/10 rounded-lg"
